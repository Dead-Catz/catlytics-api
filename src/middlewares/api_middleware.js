...
export const apiMiddleWare = (req, res, next) => {
  ...
  //helper errors:
  res.sendBadParameters = () => { res.sendError('bad parameters', 400); };
  res.sendAccessDenied = () => { res.sendError('unauthorized', 401); };
  res.sendAccessForbidden = () => { res.sendError('access denied', 403); };
  next();
};
