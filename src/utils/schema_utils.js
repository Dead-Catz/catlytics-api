...
export const ValidateData = async (schema, data, throwErrors = false) => {
    let checker = schema.apply != undefined ? schema : CreateSchemaChecker(schema);
    let isOk = checker(data);
    if (isOk)
        return true;
    if (!throwErrors) {
        return false;
    ...
    for (var i = 0; i < checker.errors.length; i++)
        errorMessage += checker.errors[i].message + '.';
    throw new Error(errorMessage);
}
export const CreateSchemaChecker = (schema) => {
    let properties = {};
    let required = [];
